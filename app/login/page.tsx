...
const Login = () => {
    ...
    const HandleLogin = async (e: FormEvent<HTMLFormElement>) => {
        e?.preventDefault()
        const LoginTostID = toast.loading("Logging in...", { toastId: "CredentialLogin" })
        setLoading(true)
        try {
        ...
        <section className='flex_center gap-8 px-4 sm:px-16 w-full h-full my-auto'>
            ...
            <div className="flex justify-around items-center flex-col gap-2 w-full sm:w-1/2 py-4">
                ...
                <form onSubmit={HandleLogin} method='POST' className='bg-baseClr py-4 sm:p-4 pt-8 flex flex-col gap-8 sm:gap-4 w-full sm:max-w-md'>
                    ...
                    {!loading ?
                        <button className='bg-primaryClr p-2 rounded text-white text-bold tracking-widest' type='submit'>
                            LOGIN
                        </button>
                        :
                ...
                <div className="flex_center flex-col gap-4 w-full sm:max-w-md sm:px-4">
                    ...
                    <button
                        ...
                        disabled={provider["google"]?.id ? false : true}
                    ...
                    <button
                        className='bg-baseClr text-textClr w-full flex_center gap-4 p-2 rounded border border-secondaryClr disabled:cursor-not-allowed'
                        disabled={provider["github"]?.id ? false : true}
