...
const UserAvatar = ({ mobileView = false }: { mobileView?: boolean }) => {
    ...
    if (!mobileView)
        return (
            <>
                ...
                {showDropMenu && <div
                    ref={DropdownRef}
                    className="bg-baseClr p-2 rounded-md absolute top-16 right-10 !w-[200px] z-20 flex flex-col gap-2 border border-secondaryClr">
        ...
        return (
            <Link href="/profile">
                {user?.image ?
                    <div className="flex_center rounded-full relative overflow-hidden" >
                        <Image src={user?.image} alt="ProfileImage" width={30} height={30} />
                    </div>
                    :
                    <div className="bg-primaryClr aspect-square text-white p-1 rounded-full" >
